...
const CreateUserModel = ({isUserModelOpen,modelTitle,onClosed,passAuthorizedList,passedListingFunction}) => {
    ...
    const handleCreateUser = async() => {
        ...
        let formValues = {};
        switch(true){
            case createPersonData.current.firstname.value == "" || createPersonData.current.lastname.value == "" || createPersonData.current.email.value == "" || createPersonData.current.role.value == "" || createPersonData.current.password.value == ""|| createPersonData.current.number.value == "" || createPersonData.current.handledSubAdmin.value == "" || createPersonData.current.status.value == "":
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <div className="absolute right-3 translate-y-2 text-green-200">
                                                ...
                                                <select name="status" id="status" className="text-white block w-full border-0 bg-transparent p-0 text-sm file:my-1 file:rounded-full file:border-0 file:bg-accent file:px-4 file:py-2 file:font-medium placeholder:text-muted-foreground/90 focus:outline-none focus:ring-0 sm:leading-7 text-foreground mt-[7px]" ref={(e)=>{createPersonData.current.status = e}}>
                                                    <option selected value="Active" className='text-black'>Active</option>
