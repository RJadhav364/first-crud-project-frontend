...
const SubAdminModel = ({toOpenModel,modelTitle,onClosed,token,handleRecallListing}) => {
    ...
    const handleCreateNewPerson = async() => {
        ...
        switch(true){
            case createPersonData.current.firstname.value == "" || createPersonData.current.email.value == "" || createPersonData.current.role.value == "" || createPersonData.current.password.value == ""|| createPersonData.current.mnumber.value == "":
            ...
            default:
                let formData = {
                    ...
                    password: createPersonData.current.password.value,
                    mnumber: createPersonData.current.mnumber.value,
                }
                // console.log(formData)
                const createAuthorizedPerson = await createNewAuthorizedPerson({body:formData,token:token});
                const data = await createAuthorizedPerson.json();
                switch(true){
                    case createAuthorizedPerson.status == 200:
                        createPersonData.current.firstname.value = '';
                        createPersonData.current.email.value = '';
            ...
            <div className="relative -mb-px h-px w-full bg-gradient-to-r from-transparent via-sky-300 to-transparent"></div>
                <div
                  ...
                  <div className="flex gap-2 p-[1rem] justify-end">
                        <Button classes={`font-semibold hover:bg-black hover:text-white hover:ring hover:ring-white transition duration-300 inline-flex items-center justify-center rounded-md text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-white text-black h-10 px-4 py-2`} btn_title={"Create"} onclickFn={handleCreateNewPerson} />
                  </div>
                </div>
            </div>
        </>
